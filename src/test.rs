...
mod tests {
    ...
    fn test_reputation_graph() {
        ...
        let object1 = ReputationProof::create(100, None);
        let pointer1 = PointerBox::ReputationProof(&object1);

        let object2 = ReputationProof::create(100000000, None);
        let pointer2 = PointerBox::ReputationProof(&object2);

        let pointer3 = PointerBox::String(String::from("nodo-c57"));

        // proof 1
        let mut proof1 = ReputationProof::create(100, Some(&pointer2));
        ...
        */
        ...
        assert_eq!((&proof1).compute(Some(&pointer1)), 0.1);
        assert_eq!((&proof1).compute(Some(&pointer2)), 0.60);
